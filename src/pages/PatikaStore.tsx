...
import CardStore from '../components/PatikaStore/CardStore/CardStore';
import Title from '../components/PatikaStore/Title/Title';
import SearchBar from '../components/PatikaStore/Search/SearchBar';

type PatikaStoreType = {
  id: number | string;
  title: string;
  imgURL: string;
  price: string;
...
const patikaStoreData: PatikaStoreType[] = [
  {
    id: 0,
...
const PatikaStore: React.FC<PatikaStoreType> = () => {
  return (
    <SafeAreaView style={styles.container}>
      <ScrollView>
        ...
        <View style={styles.cardLayout}>
          {patikaStoreData.map((item: PatikaStoreType) => {
            return <CardStore key={item.id} item={item} />;
          })}
        </View>
      </ScrollView>
    </SafeAreaView>
